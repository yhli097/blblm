...
library(parallel)
cl = makeCluster(4)
fit <- blblm(mpg ~ wt * hp, data = mtcars, m = 3, B = 100, cl = cl)
coef(fit)
...
file_names <- file.path("data/flights", list.files("data/flights"))
m <- length(file_names)
...
predict(fit, data.frame(dep_delay = c(-10,-30), flight = c(1000, 500), distance = c(1500,3000) ), confidence = TRUE)
#>       fit       lwr       upr
